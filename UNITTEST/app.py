    ...
    if doc_exist:
        for directory_number, directory_docs_list in directories.items():
            if user_doc_number in directory_docs_list:
                return user_doc_number, directory_number
    return user_doc_number, False


def move_doc_to_shelf(user_doc_number, user_shelf_number):
    exist = check_document_existance(user_doc_number)
    if exist:
        remove_doc_from_shelf(user_doc_number)
        if user_shelf_number in directories.keys():
            append_doc_to_shelf(user_doc_number, user_shelf_number)
            return True, user_doc_number, user_shelf_number
        else:
            return False, user_doc_number, user_shelf_number
    else:
        return False, user_doc_number, user_shelf_number
...
def show_document_info(document):
    ...
    strings = f'"{doc_type}" "{doc_number}" "{doc_owner_name}"'
    print(strings)
    return strings
...
def show_all_docs_info():
    ...
    for current_document in documents:
        strings = show_document_info(current_document)
        list_doc.append(strings)
    return list_doc
...
def secretary_program_start(user_command):
    ...
    while True:
        if user_command == 'p':
            owner_name = get_doc_owner_name(user_doc_number = input('Введите номер документа - '))
            return print('Владелец документа - {}'.format(owner_name))
        elif user_command == 'ap':
            uniq_users = get_all_doc_owners_names()
            return print('Список владельцев документов - {}'.format(uniq_users))
        elif user_command == 'l':
            return show_all_docs_info()
        elif user_command == 's':
            user_doc_number, directory_number = get_doc_shelf(user_doc_number = input('Введите номер документа - '))
            if directory_number:
                return print('Документ находится на полке номер {}'.format(directory_number))
            return print('Такого документа нет {}.'.format(user_doc_number))
        elif user_command == 'a':
            ...
            doc_number, deleted = delete_doc(user_doc_number = input('Введите номер документа - '))
            if deleted:
                return print('Документ с номером "{}" был успешно удален'.format(doc_number))
            return print('Вы ввели несуществующий номер документа "{}"!'.format(doc_number))
        elif user_command == 'm':
            boolean, user_doc_number, user_shelf_number = move_doc_to_shelf(user_doc_number = input('Введите номер документа - '), user_shelf_number = input('Введите номер полки для перемещения - '))
            if boolean == True:
                return  print('Документ номер "{}" был перемещен на полку номер "{}"'.format(user_doc_number, user_shelf_number))
            return print('Не правильно введены данные: "{}" или "{}"'.format(user_doc_number, user_shelf_number))
        elif user_command == 'as':
            shelf_number, added = add_new_shelf(shelf_number = input('Введите номер полки - '))
            if added:
                return print('Добавлена полка "{}"'.format(shelf_number))
...
if __name__ == '__main__':
    ...
    while True:
        if secretary_program_start(user_command = input('Введите команду - ')) == True:
            break